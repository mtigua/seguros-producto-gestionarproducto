...
@RestController
@Api(value="TipoTarifa Resource")
@RefreshScope
@RequestMapping("/tipotarifa")
public class TipoTarifaController {
	
	private static final Logger logger = LoggerFactory.getLogger(TipoTarifaController.class.getSimpleName());
	...
	private static final String SWAGGER_GET_TipoTarifa = "Listar tipo de tarifa";
	...
	@ApiOperation(value = SWAGGER_GET_TipoTarifa, notes = SWAGGER_GET_TipoTarifa)
	@ApiResponses({ 
	...
	@GetMapping("/")
	public ResponseEntity<List<TipoTarifaDto>> getTipoTarifa(
//			@RequestHeader(value = HEADER_AUTHORIZACION_KEY, required = true) 			
//			String token
			) throws TipoTarifaException, UnauthorizedException{	
				
		List<TipoTarifaDto> lista= null;
		
		try {
			//  String username=utils.getSamaccountname(token);		
			  lista= tipoTarifaService.findAll();
