...
@RestController
@Api(value="Prima sobre que Resource")
@RefreshScope
@RequestMapping("/primasobreque")
...
public class PrimaSobreQueController {
	...
	private static final String SWAGGER_GET_PrimaSobreQue = "Listar prima sobre que";
	
	@Autowired
	private PropertiesMsg propertiesMsg;	
	
	@Autowired
	private PrimaSobreQueService primaSobreQueService;
	

	@ApiOperation(value = SWAGGER_GET_PrimaSobreQue, notes = SWAGGER_GET_PrimaSobreQue)
	@ApiResponses({ 
	...
	@GetMapping("/")
	public ResponseEntity<List<PrimaSobreQueDto>> getPrimaSobreQue(	) throws PrimaSobreQueException {
		List<PrimaSobreQueDto> lista= null;

		try {
			  lista= primaSobreQueService.findAll();
		}
		catch(PrimaSobreQueException e) {
